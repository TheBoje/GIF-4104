...
void invertSequential(Matrix& iA) {
	...
	for (unsigned int i = 0; i < iA.rows(); ++i) { iA.getRowSlice(i) = lAI.getDataArray()[std::slice(i * lAI.cols() + iA.cols(), iA.cols(), 1)]; }
}
...
Matrix multiplyMatrix(const Matrix& iMat1, const Matrix& iMat2) {
	...
	for (size_t i = 0; i < lRes.rows(); ++i) {
		...
		for (size_t j = 0; j < lRes.cols(); ++j) { lRes(i, j) = (iMat1.getRowCopy(i) * iMat2.getColumnCopy(j)).sum(); }
	}
	return lRes;
}
...
int main(int argc, char** argv) {
	...
	std::cout << "Erreur: " << lRes.getDataArray().sum() - lS << std::endl;
